...
class Schedule < ActiveRecord::Base
  ...
  def self.update_schedule(season, match_id, phase_id, team1_id, team2_id, match_datetime, goal1, goal2, halfgoal1, halfgoal2, finrate, direction, result)
    schedule = where("matchyear=:season AND phase=:phase_id AND matchno=:match_id AND team1no=:team1_id AND team2no=:team2_id",
  ...
  def self.get_finished_matches(season, match_id)
    find_by_sql("select phase,team1no,team2no,halfgoal1,halfgoal2,goal1,goal2
                    from #{$tab['schedule']}
