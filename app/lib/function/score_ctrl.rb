...
class ScoreCtrl
  ...
  def self.check(args)
    flag, season, match_set = args_analyze(args)
    return unless flag
    check_score(season, match_set)
  end

  def self.asia_calc(args)
    flag, season, match_set = args_analyze(args)
    return unless flag
    calculate_asia_score(season, match_set)
  end

  def self.asia_check(args)
    flag, season, match_set = args_analyze(args)
    return unless flag
    check_asia_score(season, match_set)
  end
