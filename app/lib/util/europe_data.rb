...
module Huuuunt
  module EuropeData
    ...
    def get_new_team(xml)
      ...
      read_europe_xml(xml) do |lang|
        match_name_arr = lang.find_first('le').content # <le>友谊赛,友誼賽,INT CF</le>
        home_team_arr = lang.find_first('ta').content # <ta>瓦克蒂罗尔,華卡迪路,FC Wacker Innsbruck</ta>
        away_team_arr = lang.find_first('tb').content # <tb>基辅迪纳摩,基輔戴拿模,Dynamo Kyiv</tb>

        m_name_cn, m_name_tw, m_name_en = match_name_arr.split(",")
        h_name_cn, h_name_tw, h_name_en = home_team_arr.split(",")
        a_name_cn, a_name_tw, a_name_en = away_team_arr.split(",")

        # 判断该赛事是否要纳入统计(赛事名称无法识别的情况同样返回FALSE)
        unless MatchHelper.match_need_stat?(m_name_cn)
          unless MatchHelper.match_need_stat?(m_name_tw)
            next
          end
        end
